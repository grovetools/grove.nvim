...
local function update(bufnr)
	...
	local cmd =
		string.format("%s stats --per-line %s 2>/dev/null", vim.fn.shellescape(cx_path), vim.fn.shellescape(temp_file))
				...
				if is_rule_line then
					...
					if stat then
						...
						if stat.excludedFileCount and stat.excludedFileCount > 0 and not is_ruleset_import then
							...
							if stat.excludedTokens and stat.excludedTokens > 0 then
								table.insert(
									virt_text,
									{
										", -" .. format_compact(stat.excludedTokens) .. " tokens",
										"GroveVirtualTextExcluded",
									}
								)
							end
						end
						...
						if stat.fileCount and stat.fileCount > 0 and stat.totalTokens and stat.totalTokens > 0 then
							...
							table.insert(
								virt_text,
								{ " ~" .. format_compact(stat.totalTokens) .. " tokens", "GroveVirtualTextTokens" }
							)
						...
						elseif
							stat.fileCount == 0
							and not stat.gitInfo
							and (not stat.excludedFileCount or stat.excludedFileCount == 0)
						then
							...
							if stat.skipReason and stat.skipReason ~= "" then
								...
								elseif display_reason:match("is outside of any known or allowed workspace") then
									...
									if path then
										...
										local short_path =
											table.concat({ parts[#parts - 1] or "", parts[#parts] or "" }, "/")
										display_reason = "outside workspaces: " .. short_path
									else
										display_reason = "outside allowed workspaces"
									end
								elseif
									display_reason:match("is outside of any workspace defined in 'included_workspaces'")
								then
					...
					if #virt_text > 0 then
						...
						if ok then
							...
							vim.notify(
								"Grove: Failed to set extmark on line " .. (line_idx - 1) .. ": " .. tostring(err),
...
function M.copy_rules_with_virtual_text(bufnr)
	...
	vim.notify(
		string.format("Grove: Copied %d lines (%d with virtual text) to clipboard", #combined_lines, lines_with_vtext),
		vim.log.levels.INFO
	)
end
